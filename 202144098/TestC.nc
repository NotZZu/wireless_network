...
implementation
{
        ...
        void startDone();
        task void radioOn() {
                if (call RadioControll.start() != SUCCESS) startDone();
        }
        ...
        task void sendTask() {
                ...
                if (call RadioSend.send(AM_BROADCAST_ADDR, &testMsgBffr,
                                        sizeof(test_data_msh_t)) != SUCCESS)sendDone();
        ...
        void stopDone();
        task void radioOff() {
                if (call RadioControll.stop() != SUCCESS) stopDone();
        }
        ...
        event void Humi.readDone(error_t errorm uint16_t val) {
